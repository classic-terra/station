...
const SwapForm = ({ chainID }: { chainID: string }) => {
  ...
  const initialOfferAsset =
    (state as Token) ??
    (getAmount(bankBalance, "uusd")
      ? "uusd"
      : sortByDenom(bankBalance)[0].denom)
  ...
  const createTx = useCallback(
    (values: TxValues) => {
      ...
      return { msgs: getMsgs({ ...params, ...expected }), chainID }
    },
    [balance, calcExpected, findDecimals, getMsgsFunction, chainID]
  )
